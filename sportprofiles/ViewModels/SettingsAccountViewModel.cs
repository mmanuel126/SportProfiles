    ...
    public class SettingsAccountViewModel: INotifyPropertyChanged
    {
        ...
        public SettingsAccountViewModel(ISettings settingsSvc)
        {
            try
            {
                ...
                Task.Run(() => GetAccountSettngsInfo().Wait());
                Task.Run(() => GetAccountSettingsNotifications().Wait()); 
                IsRefreshing = false;
            }
            catch (Exception ex)
            {
        ...
        public async Task GetAccountSettngsInfo()
        {
        ...
        public async Task GetAccountSettingsNotifications()
        {
            string? jwtToken = await SecureStorage.Default.GetAsync("AccessToken");
            string memberID = "0";
            if (!String.IsNullOrEmpty(Preferences.Get("UserID", "")))
